        ...
        {displayWarnings.map((w, i) => {
          ...
          return (
            <motion.div
              ...
              className={`flex items-start gap-3 p-4 rounded-xl border ${isHigh ? "border-rose-500/20 bg-rose-500/5 border-l-2 border-l-rose-500" : "border-amber-500/15 bg-amber-500/5 border-l-2 border-l-amber-400"}`}
            >
              <span className={`w-2 h-2 rounded-full mt-2 shrink-0 ${isHigh ? "bg-rose-500" : "bg-amber-400"} animate-pulse`} />
              <div className="min-w-0 flex-1">
                <div className="text-body-sm font-semibold text-slate-200 leading-snug">{w.title}</div>
                <div className="text-caption text-slate-500 flex items-center gap-1 mt-1">
...
function TrendMiniChart({ data }: { data: any[] }) {
  ...
  return (
    <div className="card rounded-2xl">
      ...
      <div className="h-56 bg-gradient-to-b from-transparent to-slate-950/20">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: -20 }}>
            ...
            <YAxis stroke="#475569" fontSize={12} tickLine={false} allowDecimals={false} />
            <Tooltip
      ...
      <div className="flex gap-4 mt-3">
        <span className="flex items-center gap-2 text-caption text-slate-500"><span className="w-3 h-3 rounded-sm bg-rose-500/60 inline-block" />{t("missionControl.trends.heinous")}</span>
        <span className="flex items-center gap-2 text-caption text-slate-500"><span className="w-3 h-3 rounded-sm bg-amber-500/60 inline-block" />{t("missionControl.trends.nonHeinous")}</span>
      </div>
    </div>
  );
}
...
function ActivityTimeline() {
  ...
  return (
    <div className="card rounded-2xl">
      ...
      <div className="timeline-track space-y-5">
        {events.map((e, i) => (
          <motion.div
            key={i}
          ...
          >
            ...
            <div className="text-caption text-slate-500 mt-1">{e.time}</div>
          </motion.div>
        ))}
      </div>
    </div>
        ...
        {recs.map((r, i) => {
          ...
          return (
            <motion.button
              ...
              className={`w-full text-left flex items-center gap-4 p-4 rounded-xl border ${r.border} ${r.bg} group transition hover:opacity-90`}
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${r.bg} border ${r.border}`}>
                <Icon className={`w-4 h-4 ${r.color}`} />
              </div>
              <span className="text-body-sm text-slate-300 flex-1">{r.text}</span>
              <ArrowRight className={`w-4 h-4 ${r.color} shrink-0 opacity-0 group-hover:opacity-100 transition-opacity`} />
            </motion.button>
          );
        })}
      </div>
...
function QuickActions({ onNavigate }: { onNavigate: (t: string) => void }) {
  ...
  return (
    <div className="card-elevated col-span-full rounded-2xl">
      <div className="text-label text-slate-400 mb-5">{t("missionControl.quickActions.title")}</div>
        ...
        {actions.map((a, i) => {
          ...
          return (
            <motion.button
              ...
              className={`flex flex-col items-center gap-3 p-5 rounded-xl border ${a.border} bg-gradient-to-b ${a.bg} to-transparent group hover:border-opacity-60 hover:shadow-lg transition`}
            >
              <div className={`w-12 h-12 rounded-xl border ${a.border} flex items-center justify-center bg-slate-950/60`}>
