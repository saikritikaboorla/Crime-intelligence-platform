...
export default function App() {
  ...
  const handleLanguageChange = (lang: "en" | "kn") => {
    setLanguage(lang);
    logAuditEvent("Language Switch", `Changed language to ${lang}.`);
  };
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "msg_init",
      sender: "bot",
      text: "KSP Crime Intelligence core active. Ready to assist. Ask about FIR status, suspect records (e.g., 'Ramesh Kumar'), cyber phishing trails, or location analysis. Support is bilingual (English/ಕನ್ನಡ).",
  ...
  const [financialTransactions, setFinancialTransactions] = useState<(FinancialTransaction & { firNo?: string | null; caseNo?: string | null; station?: string | null; crimeType?: string | null })[]>([]);
  ...
  const handleSendMessage = async (e?: React.FormEvent, customText?: string) => {
    ...
    const queryLang: Lang = detectedLang === "kn" ? "kn" : selectedLanguage;
    if (detectedLang === "kn" && selectedLanguage !== "kn") {
      setLanguage("kn");
    }
    ...
    try {
      ...
      const botMsg: Message = {
        ...
        text: data.text,
        timestamp: new Date().toLocaleTimeString(),
        language: data.language || queryLang,
        citations: data.citations,
        confidence: data.confidence,
        groundingBasis: data.groundingBasis,
  ...
  const startVoiceInput = () => {
    ...
    if (isChatLoading) {
      showToast(t("waitQueryHint"), "info");
      return;
    }
    ...
    recognition.onresult = (event: any) => {
      ...
      if (interim) setInterimVoice(interim);
      if (finalText.trim()) {
        const captured = finalText.trim();
        setVoiceDraft(captured);
        setChatInput(captured);
        setInterimVoice("");
        setIsListening(false);
        recognitionRef.current = null;
        showToast(t("voiceCaptured"), "info");
      }
    };
  ...
  const handleFilterSearch = () => {
    ...
    if (!branch && !subBranch && !district && !station) {
      showToast(t("chooseFilterHint"), "info");
      return;
    }
  ...
  return (
    ...
    <div className="flex h-screen w-screen bg-slate-950 font-sans text-slate-300 overflow-hidden" style={{fontFamily: "'Inter', 'Segoe UI', system-ui, -apple-system, sans-serif", background: '#060d1f'}}>
      ...
      <nav className="hidden lg:flex w-80 flex-col shrink-0 h-full z-20 overflow-y-auto" style={{background: 'linear-gradient(180deg, #0a1220 0%, #080c14 100%)', borderRight: '1px solid rgba(30,39,56,0.9)'}}>
        ...
        <div className="flex-grow p-4 space-y-1 overflow-y-auto">
          ...
          {[
            ...
            { id: "profiling",      icon: UserCheck,     label: t("profiling"), desc: t("profilingDesc") },
            { id: "decision",       icon: BrainCircuit,  label: t("decision"), desc: t("decisionDesc") },
          ].map((tab) => {
            const Icon = tab.icon;
            const isSelected = activeTab === tab.id;
            return (
          ...
          {[
            ...
            { id: "financial",    icon: DollarSign,     label: t("financial"), desc: t("financialDesc") },
            { id: "forecasting",  icon: AlertTriangle,  label: t("forecasting"), desc: t("forecastingDesc") },
            { id: "heatmap",      icon: MapPin,         label: t("heatmap"), desc: t("heatmapDesc") },
          ].map((tab) => {
            const Icon = tab.icon;
            const isSelected = activeTab === tab.id;
            return (
      ...
      <div className="flex-1 flex flex-col overflow-hidden h-full">
        ...
        <main className="flex-1 overflow-hidden flex flex-col" style={{background: 'linear-gradient(180deg, #070b10 0%, #060912 100%)'}}>
          
          {/* Interactive Help & Schema Guide Overlay */}
          <AnimatePresence>
            {showHelp && (
              <motion.div
              ...
              >
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 text-xs leading-relaxed max-h-[250px] overflow-y-auto">
                  <div className="space-y-2">
                    ...
                    <p className="text-slate-400 text-micro">
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-700/60 pb-4 gap-3">
                    <div>
                      ...
                      <p className="section-subtitle mt-1">{t("chatDesc")}</p>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button onClick={toggleSpeakingState}
                        ...
                        {isSpeaking ? t("audioOn") : t("audioOff")}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0" style={{minHeight: '0'}}>
                      ...
                      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-900">
                          ...
                          <div className="space-y-1.5 text-caption">
                            {[
                              ...
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  handleLanguageChange(q.lang as any);
                                  setChatInput(q.text);
                                }}
                        ...
                        <div className="space-y-2 pt-2 border-t border-slate-800/50">
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              <option value="">{t("anyCrimeBranch")}</option>
                            ...
                            <select
                              ...
                              className="input text-xs py-2"
                            >
                              <option value="">{t("anySubBranch")}</option>
                            ...
                            <select
                              ...
                              className="input text-xs py-2"
                            >
                              <option value="">{t("anyPoliceStation")}</option>
                    ...
                    <div className="flex-grow overflow-y-auto space-y-6 p-5 scroll-smooth" style={{ scrollBehavior: 'smooth' }}>
                      {messages.map((m) => (
                        <div
                          key={m.id}
                        ...
                        >
                          {/* Avatar */}
                          <div className={`rounded-xl shrink-0 h-10 w-10 flex items-center justify-center border ${
                          ...
                          <div className="space-y-2 min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-2 text-micro text-slate-500">
                              <span className="font-bold text-slate-400 tracking-wide uppercase">
                            ...
                            }`} style={{ fontSize: 'var(--type-body-size)', lineHeight: 'var(--type-body-lh)' }}>
                              {m.text}

                              {/* Explainable AI: confidence + evidence trails on every grounded bot answer */}
                              {m.sender === "bot" && m.id !== "msg_init" && (
                                <div className="mt-3.5 pt-3.5 border-t border-slate-800/80 space-y-2">
                                  <div className="flex flex-wrap items-center gap-2">
                                    {m.groundingBasis && (
                                      <span className="inline-flex items-center gap-1 text-micro font-bold uppercase tracking-wide px-2 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/30 text-emerald-300">
                                        <Sparkles className="w-3 h-3" />
                                        {m.groundingBasis}
                                      </span>
                                    )}
                                    {typeof m.confidence === "number" && (
                                      <span className="inline-flex items-center gap-1 text-micro font-bold uppercase tracking-wide px-2 py-1 rounded-md bg-sky-500/10 border border-sky-500/30 text-sky-300">
                                  ...
                                  {m.citations && m.citations.length > 0 && (
                                    <>
                                      <div className="text-micro uppercase font-bold text-amber-500 tracking-wider flex items-center gap-1">
                                        ...
                                        {m.citations.map((cit, cIdx) => (
                                          ...
                                          >
                                            <div className="truncate">
                                              ...
                                              <div className="text-micro text-slate-500 truncate">{cit.title}</div>
                                              {cit.stationName && (
                                                <div className="text-micro text-slate-600 truncate">{cit.stationName}</div>
                                              )}
                                            </div>
                                  ...
                                  {m.reasoningPath && m.reasoningPath.length > 0 && (
                                    <div className="pt-1">
                                      <button
                                        type="button"
                                        onClick={() => setExpandedReasoningIds(prev => {
                                          const next = new Set(prev);
                                          if (next.has(m.id)) next.delete(m.id); else next.add(m.id);
                                          return next;
                                        })}
                                        className="text-micro font-bold uppercase tracking-wider text-slate-400 hover:text-amber-400 flex items-center gap-1"
                                      >
                                        {expandedReasoningIds.has(m.id) ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                                        {expandedReasoningIds.has(m.id) ? t("hideReasoning") : t("showReasoning")}
                                      </button>
                                      {expandedReasoningIds.has(m.id) && (
                                        <div className="mt-2 space-y-1.5 bg-slate-950/80 border border-slate-800 rounded-lg p-2.5">
                                          {m.reasoningPath.map((step, sIdx) => (
                                            <div key={sIdx} className="text-micro text-slate-400 font-mono leading-relaxed">
                                              <span className="text-amber-400 font-bold">{step.table}</span>
                                              {" → "}
                                              <span className="text-sky-300">{(step.fields || []).join(", ")}</span>
                                              {step.filter ? <span className="text-slate-500"> · filter: {step.filter}</span> : null}
                                              {typeof step.resultCount === "number" ? <span className="text-emerald-400"> · rows: {step.resultCount}</span> : null}
                                            </div>
                                          ))}
                                        </div>
                                      )}
                      ...
                      {isChatLoading && (
                        <div className="flex gap-3 max-w-[88%]">
                          <div className="bg-slate-900 border border-slate-800 text-slate-300 rounded-xl shrink-0 h-10 w-10 flex items-center justify-center">
                            <RefreshCw className="w-4 h-4 animate-spin text-blue-400" />
                          </div>
                          <div className="space-y-2">
                            <span className="text-micro text-slate-500 font-bold uppercase tracking-wider">KSP AI scanning database…</span>
                            <div className="bg-slate-900/60 p-4 rounded-2xl rounded-tl-none border border-slate-800 space-y-2">
                    ...
                    {/* Voice live transcript / edit-before-send panel */}
                    {(isListening || voiceDraft || interimVoice) && (
                      <div className="border-t border-amber-500/20 bg-amber-500/5 px-3 py-2 space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-micro font-bold uppercase tracking-wider text-amber-400 flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isListening ? "bg-rose-500/20 border-2 border-rose-500 animate-pulse" : "bg-amber-500/20 border-2 border-amber-500"}`}>
                              <Mic className={`w-4 h-4 ${isListening ? "text-rose-400" : "text-amber-400"}`} />
                            </div>
                            {isListening ? t("voiceListening") : t("voiceEditHint")}
                          </span>
                          {voiceDraft && (
                            <button type="button" onClick={() => { setVoiceDraft(null); setChatInput(""); setInterimVoice(""); }}
                              className="text-micro font-bold text-slate-500 hover:text-rose-400 uppercase">
                              {t("discardVoice")}
                            </button>
                          )}
                        </div>
                        ...
                        {isListening && (
                          <div className="flex items-center gap-1 h-8 bg-slate-950/50 rounded-lg px-3 py-2 border border-slate-800">
                            {[...Array(12)].map((_, i) => (
                              <div 
                                key={i} 
                        ...
                        <div className="space-y-2">
                          <div className="text-micro font-semibold text-slate-500 uppercase tracking-wide">
                            {t("liveTranscript")}
                          </div>
                          <textarea
                            value={isListening ? (interimVoice || chatInput) : chatInput}
                            onChange={(e) => { setChatInput(e.target.value); setVoiceDraft(e.target.value); }}
                            rows={4}
                            className="w-full bg-slate-950 border-2 border-amber-500/40 rounded-lg text-sm text-slate-100 p-3 focus:outline-none focus:border-amber-400 resize-y font-mono leading-relaxed"
                            placeholder={t("voiceEditHint")}
                          />
                          {isListening && interimVoice && (
                            <p className="text-micro text-slate-400 italic">Live: “{interimVoice}”</p>
                          )}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-700 max-w-lg w-full rounded-2xl p-6 space-y-4 shadow-2xl animate-scaleIn">
                      ...
                      <div className="flex justify-end pt-2">
                        ...
                        >
                          {t("openDecisionConsole")} <ArrowRight className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800/60 pb-5 gap-3 flex-shrink-0">
                  <div>
                    ...
                    <p className="section-subtitle mt-1">{t("networkSub")}</p>
                  </div>
                </div>
            ...
            {activeTab === "profiling" && (
              ...
              >
                <div>
                  ...
                  <p className="section-subtitle mt-1">{t("profilingDesc")}</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {/* Left: Repeat Offender list */}
                  <div className="card col-span-1 space-y-3">
                    <h3 className="text-label text-emerald-400">{t("habitualSuspectRoster")}</h3>
                  ...
                  {selectedOffender && (
                    <div className="lg:col-span-2 bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-5 animate-fadeIn">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 pb-3.5 gap-2">
                        <div>
                          <span className="text-micro bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded uppercase font-bold tracking-wider">
                            {t("activeCriminalProfile")}
                          </span>
                          <h3 className="text-base font-bold text-slate-100 mt-1">{selectedOffender.name}</h3>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-slate-400 font-semibold">{t("recidivismScore")}:</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                        <div className="bg-slate-900 border border-slate-850 p-3.5 rounded-xl space-y-2">
                          <h4 className="font-bold text-slate-300">{t("modusOperandiBehavior")}</h4>
                          <p className="text-slate-400 leading-relaxed italic">"{selectedOffender.modusOperandi}"</p>
                        </div>
                        <div className="bg-slate-900 border border-slate-850 p-3.5 rounded-xl space-y-2">
                          <h4 className="font-bold text-slate-300">{t("offenseFocus")}</h4>
                          <div className="flex flex-wrap gap-1.5 mt-1">
                            {selectedOffender.crimeHeads.map((ch: any, idx: number) => (
                              <span key={idx} className="bg-slate-950 text-slate-300 px-2.5 py-0.5 rounded border border-slate-800 text-micro font-semibold">
                                {ch}
                              </span>
                            ))}
                          </div>
                          <div className="pt-2">
                            <span className="text-slate-400 block font-bold">{t("knownAssociates")}:</span>
                      ...
                      <div className="space-y-3">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-amber-500">{t("recidivismDrivers")}</h4>
                      ...
                      <div className="space-y-3">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400">{t("investigativeTimeline")}</h4>
                            ...
                            <div key={idx} className="relative">
                              ...
                              <div className="flex justify-between items-start">
                                <div>
                                  ...
                                  <div className="text-micro text-slate-500 mt-0.5">{tl.date}</div>
                                </div>
                                <span className="bg-slate-900 text-slate-400 px-2 py-0.5 rounded border border-slate-850 text-micro font-bold uppercase shrink-0">
                                  {tl.status}
                                </span>
                              </div>
                            </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="section-subtitle mt-1">{t("decisionDesc")}</p>
                  </div>

                  {/* Case selector dropdown */}
                  <div className="bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-lg flex items-center gap-2">
                    <span className="text-xs text-slate-400 font-bold">{t("caseMasterRecord")}:</span>
                    <select
                ...
                {decisionSupport && (
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                      <div className="bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-3.5">
                        <div className="flex justify-between items-start flex-wrap gap-2">
                          <div>
                            <span className="text-micro bg-amber-500/10 text-amber-400 px-2 py-0.5 rounded border border-amber-500/20 font-bold uppercase">
                              {t("firDetailsDossier")}
                            </span>
                        ...
                        <div className="bg-slate-900 border border-slate-850 p-4 rounded-xl text-xs text-slate-300 leading-relaxed italic">
                          <strong>{t("incidentBriefFacts")}:</strong> "{decisionSupport.brief}"
                        </div>

                        <div className="pt-2">
                          <span className="text-xs text-slate-400 font-bold block mb-2">{t("accusedListedInCase")}:</span>
                          <div className="flex flex-wrap gap-2">
                            {decisionSupport.accusedList.map((a: any, idx: number) => (
                              <span key={idx} className={`border px-2.5 py-1 rounded-lg text-xs font-semibold ${a.isRepeat ? "bg-red-900/30 border-red-700/50 text-red-300" : "bg-slate-900 border-slate-800 text-slate-200"}`}>
                                ...
                                {a.isRepeat && <span className="ml-1 text-micro text-red-400 font-bold uppercase">REPEAT</span>}
                              </span>
                            ))}
                          </div>
                        </div>
                      ...
                      <div className="bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-4">
                        <h3 className="text-xs font-bold uppercase tracking-wider text-amber-500">{t("chronologicalTimeline")}</h3>
                        <div className="border-l border-slate-800 ml-2.5 pl-5 space-y-4 text-xs">
                          {decisionSupport.timeline.map((tl: any, idx: number) => (
                            <div key={idx} className="relative">
                              ...
                              <div className="text-micro text-slate-500 mt-0.5">{new Date(tl.time).toLocaleString()}</div>
                    ...
                    <div className="space-y-6 col-span-1">
                      ...
                      {/* Similar Past cases */}
                      <div className="bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-4">
                        <h3 className="text-xs font-bold uppercase tracking-wider text-sky-400">{t("precedentCases")}</h3>
                          ...
                          <div className="space-y-3">
                            {decisionSupport.similarCases.map((sim: any, idx: number) => (
                              ...
                              >
                                ...
                                <span className="inline-block bg-slate-950 text-micro font-bold text-slate-500 border border-slate-900 px-1.5 py-0.5 rounded uppercase mt-1">
                                  Outcome: {sim.status}
                                </span>
                              </button>
                            ))}
              ...
              >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2 border-t border-slate-800/60">
                    {[
                      ...
                      <div key={tx.id} className={`bg-slate-900/60 border border-slate-800 rounded-lg p-3 space-y-1.5`}>
                        ...
                        <div className="text-micro text-slate-500 font-mono">{tx.time}</div>
                        <div className={`text-sm font-extrabold tabular-nums ${
                          tx.color === "rose" ? "text-rose-400" : tx.color === "amber" ? "text-amber-400" : tx.color === "emerald" ? "text-emerald-400" : "text-purple-400"
                        }`}>{tx.amt}</div>
                        <div className="text-micro font-mono text-slate-400 truncate">{tx.from}</div>
                        <div className="text-micro text-slate-500 text-center">↓</div>
                        <div className={`text-micro font-mono font-bold truncate ${
                          tx.color === "rose" ? "text-rose-300" : tx.color === "amber" ? "text-amber-300" : tx.color === "emerald" ? "text-emerald-300" : "text-purple-300"
                        }`}>{tx.to}</div>
                        <div className="text-micro text-slate-500 leading-relaxed pt-1 border-t border-slate-800/60">{tx.note}</div>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  const categoryAgg = [
                    {
                      ...
                      count: financialTransactions.filter((t) => t.RiskReason?.toLowerCase().includes("mule") || t.RiskReason?.toLowerCase().includes("phishing") || t.RiskReason?.toLowerCase().includes("layering")).length,
                      fill: "#ef4444",
                    },
                    {
                      ...
                      amount: financialTransactions
                        ...
                        .reduce((sum, t) => sum + t.Amount, 0),
                      count: financialTransactions.filter((t) => t.RiskReason?.toLowerCase().includes("drug") || t.RiskReason?.toLowerCase().includes("narcotics") || t.RiskReason?.toLowerCase().includes("mdma")).length,
                      fill: "#f59e0b",
                    },
                    {
                      ...
                      amount: financialTransactions
                        ...
                        .reduce((sum, t) => sum + t.Amount, 0),
                      count: financialTransactions.filter((t) => t.RiskReason?.toLowerCase().includes("extortion") || t.RiskReason?.toLowerCase().includes("hawala")).length,
                      fill: "#8b5cf6",
                    },
                    {
                      ...
                      amount: financialTransactions
                        ...
                        .reduce((sum, t) => sum + t.Amount, 0),
                      count: financialTransactions.filter((t) => t.RiskReason?.toLowerCase().includes("embezzled") || t.RiskReason?.toLowerCase().includes("sim swap") || t.RiskReason?.toLowerCase().includes("job fraud")).length,
                      fill: "#0284c7",
                    },
                    {
                      ...
                      amount: financialTransactions
                        ...
                        .reduce((sum, t) => sum + t.Amount, 0),
                      count: financialTransactions.filter((t) => t.RiskReason?.toLowerCase().includes("arms") || t.RiskReason?.toLowerCase().includes("robbery") || t.RiskReason?.toLowerCase().includes("murder")).length,
                      fill: "#10b981",
                    },
                  ];
                ...
                <div className="bg-slate-950/90 border border-slate-800 rounded-xl p-6 space-y-4 shadow-xl">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
                    <div>
                      <h3 className="text-micro font-bold uppercase tracking-wider text-sky-400 flex items-center gap-2">
                  ...
                  {(() => {
                    const filteredTxs = financialTransactions.filter((t) => {
                      if (finRiskFilter === "SUSPICIOUS" && !t.IsSuspicious) return false;
                      if (finRiskFilter === "NORMAL" && t.IsSuspicious) return false;
                      if (finSearchQuery) {
                        const q = finSearchQuery.toLowerCase();
                      ...
                      <div className="relative overflow-x-auto rounded-xl border border-slate-800 bg-slate-950 p-2">
                        ...
                        {finHoveredEdge && (
                          <div className="absolute top-4 right-4 z-20 bg-slate-900/95 border border-slate-700 p-3.5 rounded-xl shadow-2xl max-w-sm text-xs space-y-1 text-slate-200">
                            ...
                            <div className="text-slate-400 text-micro">
                              Sender: {finHoveredEdge.sender} → Recipient: {finHoveredEdge.recipient}
                            </div>
                            {finHoveredEdge.reasons.length > 0 && (
                              <div className="text-rose-300/90 italic text-micro pt-1 border-t border-slate-800">
                ...
                <div className="bg-slate-950/80 border border-slate-800 p-6 rounded-xl space-y-4 shadow-xl">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
                    ...
                    <span className="text-xs text-slate-400 font-semibold">
                      Showing {financialTransactions.length} records from FIR Financial Database
                    </span>
                  </div>
                            ...
                            return (
                              <tr key={idx} className={`${rowBg} ${borderColor} hover:bg-slate-850/80 transition-colors`}>
                                ...
                                <td className="py-3 px-4 text-center whitespace-nowrap">
                                  {isSusp ? (
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-micro font-extrabold uppercase bg-rose-500/20 text-rose-300 border border-rose-500/40">
                                      Flagged
                                    </span>
                                  ) : (
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-micro font-bold uppercase bg-slate-800 text-slate-400 border border-slate-700">
                                      Passed
                                    </span>
                                  )}
                                </td>
                  ...
                  {forecasting.warnings && forecasting.warnings.map((w: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-4 flex flex-col justify-between">
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            <span className={`text-micro font-black uppercase px-2 py-0.5 rounded border tracking-wider ${sevColor}`}>
                              {w.severity} SEVERITY ({w.confidence}% Conf)
                            </span>
                            <span className="text-micro text-slate-500 font-bold">ALARM {w.id}</span>
                          </div>
                          
                          <h3 className="text-heading2 text-slate-100">{w.title}</h3>
                          
                          <div className="flex items-center gap-2 text-body-sm text-slate-400 font-medium">
                            <MapPin className="w-4 h-4 text-rose-500 shrink-0" />
                            <span>{w.location}</span>
                          </div>

                          <div className="bg-slate-900 border border-slate-850 p-4 rounded-xl text-body-sm text-slate-300 leading-relaxed italic">
                            <strong>AI Grounding Reasoning:</strong> "{w.reasoning}"
                          </div>
                        </div>

                        <div className="pt-3 border-t border-slate-800/80 space-y-2">
                          <strong className="text-amber-500 uppercase tracking-widest text-micro block">Proposed Preventive Action Plan:</strong>
                          <p className="text-body-sm text-slate-400 leading-relaxed">{w.actionProposed}</p>
                          <button
                            onClick={() => {
                            ...
                            className="w-full mt-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold py-1.5 px-3 rounded-lg text-micro transition"
                          >
            ...
            {activeTab === "audit" && (
              ...
              >
                <div>
                  ...
                  <p className="section-subtitle mt-1">{t("auditDesc")}</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {/* Left Column: Governance Info */}
                  <div className="bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-4 col-span-1">
                    <h3 className="text-micro font-bold uppercase tracking-wider text-emerald-400">{t("dataProtectionCompliance")}</h3>
                    <p className="text-body-sm leading-relaxed text-slate-300">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start gap-2 text-slate-400">
                        ...
                        <span className="text-body-sm">All personal identification records are masked beyond necessary investigative access lines.</span>
                      </div>
                      <div className="flex items-start gap-2 text-slate-400">
                        <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                        <span className="text-body-sm">Every natural language chat session generates a cryptographic, non-repudiable audit log instantly.</span>
                      </div>
                      <div className="flex items-start gap-2 text-slate-400">
                        <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                        <span className="text-body-sm">Judicial-level evidence tracking ensures citation reference accuracy.</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-2 bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-4">
                    <div className="flex justify-between items-center">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-amber-500">{t("liveAuditVaultLog")}</h3>
                      <button
                        onClick={() => {
                          setAuditLogs([]);
                          logAuditEvent("System Audit Clear", "Audit view reset by administrator.");
                        }}
                        className="text-xs text-rose-400 hover:text-rose-300 transition font-bold uppercase text-micro"
                      >
                        {t("resetLocalView")}
                      </button>
                    </div>
                    ...
                    <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                      {auditLogs.map((log) => (
                        <div key={log.id} className="bg-slate-900/60 border border-slate-800/50 border-l-2 border-l-emerald-500/60 p-3.5 rounded-xl space-y-1.5">
                          <div className="flex justify-between items-center">
                            <span className="badge badge-slate" style={{ fontSize: 'var(--type-meta-size)' }}>
                              {log.userRole}
                            </span>
                            <span className="text-slate-500 font-mono" style={{ fontSize: 'var(--type-meta-size)' }}>{new Date(log.timestamp).toLocaleTimeString()}</span>
                          </div>
                          
                          <div className="flex items-start gap-1.5 flex-wrap">
                            <span className="font-bold text-emerald-400" style={{ fontSize: 'var(--type-body-size)' }}>{log.actionType}:</span>
                            <span className="text-slate-300" style={{ fontSize: 'var(--type-body-size)', lineHeight: '1.6' }}>{log.details}</span>
                          </div>

                          {log.query && (
                            <p className="bg-slate-950/80 p-2 rounded-lg border border-slate-800/60 text-slate-500 font-mono" style={{ fontSize: 'var(--type-meta-size)' }}>
                              Query: "{log.query}"
                            </p>
                          )}
                        </div>
        ...
        {showLogoutConfirm && (
          ...
          >
            ...
            >
              ...
              {/* Text */}
              <h2 id="logout-title" className="text-heading3 text-slate-100 mb-2">{t("signOutConfirmTitle")}</h2>
